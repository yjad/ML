...
def classification_performance(y_test, pred):
    ...
    s += out_string( 'classification_report: \n', cr)
    cm = confusion_matrix(y_test, pred)
    s += out_string('confusion_matrix: for each classification, how may correct, how many incorrect:\n', cm )

    s += out_string('Accuracy score:', accuracy_score(y_test, pred) )
...
def run_classification(X_train, X_test, y_train, y_test, model_id=None):
    ...
    for reg_model_id, reg_fn in enumerate(cfcn_functions):
        ...
        if model_id is not None and (reg_model_id != model_id):
            continue
        out += out_string (20*'*', cfcn_method[reg_model_id],20*'*' )
    ...
    out += out_string (20*'*', " Summary ", 20*'*')
    out += out_string  (f"Best model: {best_model} with accuracy: {best_accuracy:.2f}" )
...
def use_model(model_name, X_test):

    with open(f'{path.join(DATA_FOLDER, model_name)}.pkl', 'rb') as fid:
            cfn_model = pickle.load(fid) 
    sc = StandardScaler()
    # X_test = sc.transform(X_test)
    pred = cfn_model.predict(X_test)
